...
//const do_spm_t do_spm = (do_spm_t)(0x7c18 >> 1);
#endif
...
void optiboot_readPage(uint8_t storage_array[], uint16_t page)
{
  uint8_t read_character;
  for (uint16_t j = 0; j < SPM_PAGESIZE; j++)
  {
    read_character = pgm_read_byte((optiboot_addr_t)(void*) SPM_PAGESIZE * page + j);
...
void optiboot_writePage(uint8_t data_to_store[], uint16_t page)
{
  uint16_t word_buffer = 0;

  // Erase the flash page
  optiboot_page_erase((optiboot_addr_t)(void*) SPM_PAGESIZE * page);
  ...
  for (uint16_t i = 0; i < SPM_PAGESIZE; i++)
  {
    ...
    else
    {
      ...
      optiboot_page_fill((optiboot_addr_t)(void*)  SPM_PAGESIZE * page + i, word_buffer);
    }
  }

  // Writing temporary buffer to flash
  optiboot_page_write((optiboot_addr_t)(void*) SPM_PAGESIZE * page);
}
